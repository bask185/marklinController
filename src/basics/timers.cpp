...
volatile unsigned char ACcontrolT;
volatile unsigned char throttleT;
volatile unsigned char rebootT;
volatile unsigned char shortCircuitT;
...
_1ms += 1;
	...
	if(shortCircuitT) shortCircuitT--;
...
if(_1ms == 10) { _1ms = 0; _10ms += 1;
	...
	if(ACcontrolT) ACcontrolT--;
	if(throttleT) throttleT--;
